...
*/
  ...
  song = minim.loadFile("C:/Users/Ron/Downloads/The Strokes - Reptilia.mp3", 512);
  song.play();
...
void copyArray(byte[] from, byte[] to, boolean transparent) {
...
void spc(int x, int y, int c, byte[] arr) {
  if (0 <= y && y < h && 0 <= x && x < w) {
    if (y % 2 == 0) {
...
void approxWaveform() {
  ...
  for(int i = 0; i < w; i++) {
    ...
    for (int j = 0; j < h; j++) {
      ...
      cc.colourScale(1/pow((y-j)*(y-j), 1));
      o.colourAdd(cc);
      spc(i,j, o, waveform);
    } 
  } 
  ...
  */
  ...
  for (i = 0; i < w; i++) {
    for (j = 0; j < h; j++) {
      ...
      cc = new Colour(c.returnInt());
      cc.colourScale(1/pow((x-i)*(x-i) + (y-j)*(y-j), 0.75));
      o.colourAdd(cc);
      spc(i,j, o, bg);
    } 
  }
